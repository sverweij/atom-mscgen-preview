...
describe "Markdown preview package", ->
  ...
  describe "when a preview has not been created for the file", ->
    it "displays a mscgen preview in a split pane", ->
      waitsForPromise -> atom.workspace.open("subdir/file.mscgen")
  ...
  describe "when a preview has been created for the file", ->
    ...
    describe "when a new grammar is loaded", ->
      it "re-renders the preview", ->
        ...
        waitsFor "mscgen to be rendered after its text changed", ->
        ...
        waitsFor "mscgen to be rendered after grammar was added", ->
  ...
  describe "when the mscgen preview view is requested by file URI", ->
    it "opens a preview editor and watches the file for changes", ->
      waitsForPromise "atom.workspace.open promise to be resolved", ->
        atom.workspace.open("mscgen-preview://#{atom.project.getDirectories()[0].resolve('subdir/file.mscgen')}")
  ...
  describe "when the editor's grammar it not enabled for preview", ->
    it "does not open the mscgen preview", ->
      atom.config.set('mscgen-preview.grammars', [])

      waitsForPromise ->
        atom.workspace.open("subdir/file.mscgen")
  ...
  describe "when the editor's path changes on #win32 and #darwin", ->
    it "updates the preview's title", ->
      ...
      runs ->
        expect(preview.getTitle()).toBe 'file.mscgen Preview'
  ...
  describe "when the mscgen contains an <html> tag", ->
  ...
  describe "when the mscgen contains a <pre> tag", ->
    it "does not throw an exception", ->
  ...
  describe "GitHub style mscgen preview", ->
    beforeEach ->
      atom.config.set 'mscgen-preview.useGitHubStyle', false

    it "renders mscgen using the default style when GitHub styling is disabled", ->
    ...
    it "renders mscgen using the GitHub styling when enabled", ->
      atom.config.set 'mscgen-preview.useGitHubStyle', true
