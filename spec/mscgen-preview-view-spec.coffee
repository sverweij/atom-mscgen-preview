...
describe "MscGenPreviewView", ->
  ...
  beforeEach ->
    filePath = atom.project.getDirectories()[0].resolve('subdir/file.mscgen')
  ...
  describe "::constructor", ->
    it "shows a loading spinner and renders the mscgen", ->
      preview.showLoading()
      expect(preview.find('.mscgen-spinner')).toExist()
  ...
  describe "serialization", ->
    ...
    it "does not recreate a preview when the file no longer exists", ->
      filePath = path.join(temp.mkdirSync('mscgen-preview-'), 'foo.md')
  ...
  describe "gfm newlines", ->
    describe "when gfm newlines are not enabled", ->
      it "creates a single paragraph with <br>", ->
        atom.config.set('mscgen-preview.breakOnSingleNewline', false)
    ...
    describe "when gfm newlines are enabled", ->
      it "creates a single paragraph with no <br>", ->
        atom.config.set('mscgen-preview.breakOnSingleNewline', true)
  ...
  describe "when core:save-as is triggered", ->
    ...
    it "saves the rendered HTML and opens it", ->
      ...
      markdownPreviewStyles = [
        {
          rules: [
            createRule ".mscgen-preview", "{ color: orange; }"
          ]
        }, {
          rules: [
            createRule ".not-included", "{ color: green; }"
            createRule ".mscgen-preview :host", "{ color: purple; }"
          ]
        }
      ]
      ...
      atomTextEditorStyles = [
        ...
        "atom-text-editor .hr { background: url(atom://mscgen-preview/assets/hr.png); }"
      ]
